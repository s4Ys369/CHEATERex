...
u32 interact_star_or_key(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    u8 stayInLevelCommon = (configStayInLevel > 0 && !(m->controller->buttonDown & L_TRIG) &&
        !(gCurrLevelNum == LEVEL_BOWSER_1 || gCurrLevelNum == LEVEL_BOWSER_2 || gCurrLevelNum == LEVEL_BOWSER_3));
    if (configStayInLevel > 0 && stayInLevelCommon == TRUE) {
        u32 noExit = TRUE;
    }
    ...
    if (m->health >= 0x100) {
        ...
        if (Cheats.NonStop == FALSE && configStayInLevel == 0 || stayInLevelCommon == FALSE) {
