...
static u32 sSpeedrunTimerStart = 0;
static u32 sSpeedrunTimerEnd = 0;

void time_trials_speedrun_init() {
    sSpeedrunTimerStart = gGlobalTimer;
    sSpeedrunTimerEnd = 0;
    configSkipIntro = FALSE;
    gCLIOpts.SkipIntro = FALSE;
    save_file_erase(3);
}

void time_trials_speedrun_end() {
    if (sSpeedrunTimerStart != 0) {
        sSpeedrunTimerEnd = gGlobalTimer;
    }
}

void time_trials_speedrun_deinit() {
    sSpeedrunTimerStart = 0;
    sSpeedrunTimerEnd = 0;
}

s32 time_trials_speedrun_is_started() {
    return sSpeedrunTimerStart != sSpeedrunTimerEnd;
}
...
const u8 gTimeTrialsTextSpeedrun[]           = { S P E E D R U N                         DIALOG_CHAR_TERMINATOR };
...
static void time_trials_speedrun_render_timer(s16 x, s16 y, u32 time, u8 colorFade) {
    time = MIN(time + TIME_TRIALS_SPEEDRUN_TIMER_START, TIME_TRIALS_SPEEDRUN_MAX_TIME);
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_begin);
    gDPSetEnvColor(gDisplayListHead++, colorFade, colorFade, colorFade, 255);
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x),       (y),     sTimeTrialTextTime);
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x -  60), (y),     time_trials_int_to_sm64_string((time / 108000), "%0d"));
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x -  72), (y - 8), sTimeTrialTextDot);
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x -  72), (y),     sTimeTrialTextDot);
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x -  84), (y),     time_trials_int_to_sm64_string((time / 1800) % 60, "%02d"));
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x - 108), (y - 8), sTimeTrialTextDot);
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x - 108), (y),     sTimeTrialTextDot);
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x - 120), (y),     time_trials_int_to_sm64_string((time / 30) % 60, "%02d"));
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x - 144), (y),     sTimeTrialTextDot);
    print_hud_lut_string(HUD_LUT_GLOBAL, GFX_DIMENSIONS_RECT_FROM_RIGHT_EDGE(x - 156), (y),     time_trials_int_to_sm64_string(((time * 10) / 3) % 100, "%02d"));
    gSPDisplayList(gDisplayListHead++, dl_rgba16_text_end);
    gDPSetEnvColor(gDisplayListHead++, 255, 255, 255, 255);
}

void time_trials_render_hud_timer() {
    if (TIME_TRIALS_SPEEDRUN) {
        if (sSpeedrunTimerEnd > sSpeedrunTimerStart) {
            u8 colorFade = sins(gGlobalTimer * 0x800) * 50.f + 205.f;
            time_trials_speedrun_render_timer(320, 213, sSpeedrunTimerEnd - sSpeedrunTimerStart, colorFade);
        } else {
            time_trials_speedrun_render_timer(320, 213, gGlobalTimer - sSpeedrunTimerStart, 255);
        }
    } else {
